...
namespace NBsys{namespace NDijkstra
{
	...
	*/
	...
	/** 接続。
	*/
	void Dijkstra::Connect(s32 a_node_index_a,s32 a_node_index_b)
	{
		Dijkstra_Node& t_node_a = this->node_pool.at(a_node_index_a);
		Dijkstra_Node& t_node_b = this->node_pool.at(a_node_index_b);

		s32 t_cost = static_cast<s32>((t_node_a.pos - t_node_b.pos).Length());

		this->connect_pool.push_back(Dijkstra_Connect(a_node_index_b,t_cost));
		s32 t_connect_index_a = this->connect_pool.size() - 1;
		t_node_a.connect_index_list.push_back(t_connect_index_a);

		this->connect_pool.push_back(Dijkstra_Connect(a_node_index_a,t_cost));
	...
	/** ノード追加。
	*/
	s32 Dijkstra::AddNode(const NGeometry::Geometry_Vector3& a_pos)
	{
		this->node_pool.push_back(Dijkstra_Node(a_pos));
	...
	/** ノードインデックス取得。
	*/
	s32 Dijkstra::GetNodeIndexFromPos(const NGeometry::Geometry_Vector3& a_pos)
	{
		s32 t_index = -1;
		f32 t_length_min = -1;

		s32 t_node_index = 0;
		STLVector< Dijkstra_Node >::iterator t_it_end = this->node_pool.end();
		for(STLVector< Dijkstra_Node >::iterator t_it = this->node_pool.begin();t_it != t_it_end;++t_it){
	...
	/** 経路探査。
	*/
	NGeometry::Geometry_Vector3 Dijkstra::SearchRoot(const NGeometry::Geometry_Vector3& a_start,const NGeometry::Geometry_Vector3& a_end)
	{
		//計算結果代入。
		sharedptr< Dijkstra_CalcTemp > t_calctemp_list_ptr(new Dijkstra_CalcTemp[this->node_pool.size()],default_delete< Dijkstra_CalcTemp[] >());
		Dijkstra_CalcTemp* t_calctemp_list = t_calctemp_list_ptr.get();
		...
		while(t_current_list->size()){
			...
			for(STLVector< s32 >::iterator t_it_node = t_current_list->begin();t_it_node != t_it_node_end;++t_it_node){
				...
				Dijkstra_Node& t_node = this->node_pool.at(*t_it_node);
				Dijkstra_CalcTemp& t_calctemp = t_calctemp_list[*t_it_node];
				...
				for(STLVector< s32 >::iterator t_it_connect = t_connect_index_list.begin();t_it_connect != t_it_connect_end;++t_it_connect){
					...
					Dijkstra_Connect& t_connect = this->connect_pool.at(*t_it_connect);
					
					s32 t_total_cost = t_connect.cost + t_calctemp.cost;

					Dijkstra_CalcTemp& t_to_calctemp = t_calctemp_list[t_connect.node_index];
